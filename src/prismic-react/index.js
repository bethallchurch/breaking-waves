/**
 * This file is the prismic specific code needed for the
 * project. This file can be left alone and won't likely
 * need to be changed.
 *
...
function UniversalComponent ({ request: fnFetchData, component: WrappedComponent }) {
  if (!fnFetchData) throw new Error("[Prismic UniversalComponent] Missing parameter 'request' - must be an async call returning a Promise.")
  if (!WrappedComponent) throw new Error("[Prismic UniversalComponent] Missing parameter 'component' - must be a valid React Component")

  return class extends React.Component {

    constructor (props) {
      super(props)
      if (!props.PRISMIC_UNIVERSAL_DATA) throw new Error(`[Prismic - Component '${WrappedComponent.name}'] You must provide a props 'PRISMIC_UNIVERSAL_DATA'.`)
      if (!props.prismicCtx) throw new Error(`[Prismic - Component '${WrappedComponent.name}'] You must provide a props 'prismicCtx' which contains at least the object 'api' and the api endpoint.`)
      if (!props.prismicCtx.api) throw new Error(`[Prismic - Component '${WrappedComponent.name}'] You must provide a props 'prismicCtx' which contains at least the object 'api' and the api endpoint.`)
      if (!props.prismicCtx.endpoint) throw new Error(`[Prismic - Component '${WrappedComponent.name}'] You must provide a props 'prismicCtx' which contains at least the object 'api' and the api endpoint.`)
      this.state = {
