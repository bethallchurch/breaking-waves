...
app.get('*', (req, res, next) => {
  ...
  Promise.all([
    ...
    res.locals.ctx.api.getSingle('footer')
  ])
  .then(function (result) {
    const menu = result.find(document => document.type === 'menu')
    const footer = result.find(document => document.type === 'footer')
    routes(res.locals.ctx).some(route => {
      // use `matchPath` here
      const match = matchPath(req.url, route)
      if (match) makeAsyncRender(req, res, next, route, match, menu, footer)
      return match
    })
  })
})
...
function makeAsyncRender (req, res, next, route, match, menu, footer) {
  ...
  .then((data = {}) => {
    const PRISMIC_UNIVERSAL_DATA = { [req.url]: data, menu, footer }
    const context = {}
